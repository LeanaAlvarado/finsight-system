import { supabase, peso, escapeHtml, formatDate, insertWithOptionalColumns, updateWithOptionalColumns } from "./supabase.js?v=20260813-revenue-combined-downpayments-v195";
  ...
  const html = `
      <div class="letterhead">
        ...
        <div class="quote-meta">
          ...
          <div>${escapeProjectHtml(project.project_title || "-")}</div>
...
window.editProject = async function(id) {
  ...
  if (editManpowerProjectBudget) editManpowerProjectBudget.value = Number(project.project_budget || 0) ? Number(project.project_budget || 0) : "";
...
window.generatePPR = async function(id) {
  ...
  const feedbackEntries = feedbacks
    .slice()
    .sort((a, b) => new Date(b.created_at || b.date || 0) - new Date(a.created_at || a.date || 0));
  const feedbackDetailsHtml = feedbackEntries.length
    ? feedbackEntries.map((feedback, index) => `
        <div class="feedback-entry">
          <strong>Client Feedback ${feedbackEntries.length > 1 ? String(index + 1).padStart(2, "0") : ""}</strong>
          <p>${escapeProjectHtml(feedback.comments || "No client feedback comment provided.")}</p>
          <strong>Recommendation</strong>
          <p>${escapeProjectHtml(feedback.recommendations || "No recommendation provided.")}</p>
        </div>
      `).join("")
    : `
      <div class="feedback-entry">
        ...
        <strong>Recommendation</strong>
        <p>No recommendation has been submitted for this project.</p>
      </div>
    `;
    ...
    <section class="ppr-page">
      ...
      <div class="feedback-qr-grid">
        <div class="section-card">
          <h3>Client Feedback and Recommendation</h3>
          ${feedbackDetailsHtml}
        </div>
        <div class="qr-card">
          <img src="${qrUrl}" alt="Client feedback QR code">
          <strong>Client Feedback QR</strong>
          <p>Scan the QR code to submit project feedback.</p>
    ...
    <head>
      ...
      <style>
        ...
        *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
        ...
        .cover-main{text-align:center;padding:18mm 0 9mm;}
        ...
        .feedback-qr-grid{grid-template-columns:2fr 1fr;margin-top:8px;align-items:start;}
        .feedback-entry{border:1px solid #DCE4EA;background:#fff;border-radius:6px;padding:8px;margin-top:8px;}
        .feedback-entry:first-of-type{margin-top:0;}
        .feedback-entry strong{display:block;color:#12304A;font-size:10px;text-transform:uppercase;margin-top:5px;}
        .feedback-entry strong:first-child{margin-top:0;}
        ...
        .ppr-footer{position:absolute;left:13mm;right:13mm;bottom:7mm;display:flex;justify-content:space-between;border-top:1px solid #DCE4EA;padding-top:5px;color:#64748B;font-size:8px;}
